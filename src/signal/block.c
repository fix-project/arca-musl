...
void __block_all_sigs(void *set) {
  __sigproc_mask(SIG_BLOCK, (sigset_t *)&all_mask, set);
}

void __block_app_sigs(void *set) {
  __sigproc_mask(SIG_BLOCK, (sigset_t *)&app_mask, set);
}

void __restore_sigs(void *set) { __sigproc_mask(SIG_SETMASK, set, 0); }

int __sigproc_mask(int how, const sigset_t *set, sigset_t *oldset) {
  // TODO (arca): sigproc_mask
  return -1;
}
