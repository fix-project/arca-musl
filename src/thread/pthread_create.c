...
hidden int __set_robust_list(volatile void *head, size_t len) {
  // TODO (arca): __set_robust_list
  return -1;
}
...
_Noreturn void __pthread_exit(void *result) {
  ...
  if (state == DT_DETACHED && self->map_base) {
    ...
    if (self->robust_list.off)
      __set_robust_list(0, 3 * sizeof(long));
...
static int start(void *p) {
  ...
  if (state) {
    ...
    if (args->control) {
      __set_tid_address(&args->control);
      for (;;)
        _Exit(0);
    }
  }
  __sigproc_mask(SIG_SETMASK, (const sigset_t *)&args->sig_mask, 0);
...
int __pthread_create(pthread_t *restrict res,
  ...
  if (!libc.threaded) {
    ...
    __sigproc_mask(SIG_UNBLOCK, SIGPT_SET, 0);
  ...
  if (attr._a_stackaddr) {
    ...
    size =
        guard + ROUND(attr._a_stacksize + libc.tls_size + __pthread_tsd_size);
  }
  ...
  if (ret < 0) {
    ...
    ret = sched_setscheduler(
        new->tid, attr._a_policy, (const struct sched_param *)&attr._a_prio);
